...
A Flutter package for multi-item selection with drag, range, and rectangle selection.
...
- **Rectangle selection** - Draw selection rectangles over multiple items
...
### Rectangle Selection
Draw selection rectangles over multiple items:
...
Display rectangle selection feedback:

```dart
Stack(
  children: [
    SelectionCanvas(child: GridView(...)),
    SelectionRectangleOverlay(), // Shows rectangle during selection
  ],
)
```
...
- **Grid Selection Demo** - Photo grid with drag and rectangle selection
